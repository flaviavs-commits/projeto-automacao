...
PULL_POLICY="${LLM_PULL_POLICY:-if_missing}"
...
if [ "${PULL_POLICY}" = "never" ]; then
  echo "[llm-runtime] pull policy=never, skipping model pulls"
else
  echo "[llm-runtime] ensuring models: ${MODELS_TO_PULL} (policy=${PULL_POLICY})"
  for RAW_MODEL in $(echo "${MODELS_TO_PULL}" | tr ',' ' '); do
    ...
    if [ "${PULL_POLICY}" = "if_missing" ] && ollama list | awk 'NR>1 {print $1}' | grep -Fxq "${MODEL_NAME}"; then
